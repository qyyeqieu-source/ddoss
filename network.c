...
// ===== V18_TLS RANDOMIZATION HELPERS (GeoIP Bypass Enhancement) =====

void randomize_user_agent(Connection *conn) {
    int idx = fast_rand() % USER_AGENT_COUNT;
    strncpy(conn->randomized_ua, user_agent_pool[idx], sizeof(conn->randomized_ua) - 1);
    conn->randomized_ua[sizeof(conn->randomized_ua) - 1] = '\0';
}

void randomize_h2_settings(Connection *conn) {
    // Generate variant-based H2 SETTINGS frame with randomization
    conn->h2_settings_variant = fast_rand() % H2_SETTINGS_VARIANTS;
    unsigned char *settings = conn->h2_settings;
    int *len = &conn->h2_settings_len;
    
    // Frame header: [length:3][type:1][flags:1][stream_id:4]
    settings[0] = 0x00; settings[1] = 0x00; settings[2] = 0x18;
    settings[3] = 0x04; // SETTINGS frame type
    settings[4] = 0x00; // flags
    settings[5] = 0x00; settings[6] = 0x00; settings[7] = 0x00; settings[8] = 0x00;
    
    int offset = 9;
    
    // Setting 1: HEADER_TABLE_SIZE (0x0001)
    settings[offset++] = 0x00; settings[offset++] = 0x01;
    int header_size = 2048 + (fast_rand() % 6144); // 2048-8192
    settings[offset++] = (header_size >> 8) & 0xFF; settings[offset++] = header_size & 0xFF;
    
    // Setting 2: ENABLE_PUSH (0x0002)
    settings[offset++] = 0x00; settings[offset++] = 0x02;
    int enable_push = fast_rand() % 2; // 0 or 1
    settings[offset++] = 0x00; settings[offset++] = enable_push & 0xFF;
    
    // Setting 3: MAX_CONCURRENT_STREAMS (0x0003)
    settings[offset++] = 0x00; settings[offset++] = 0x03;
    int max_streams = 100 + (fast_rand() % 900); // 100-1000
    settings[offset++] = (max_streams >> 8) & 0xFF; settings[offset++] = max_streams & 0xFF;
    
    // Setting 4: INITIAL_WINDOW_SIZE (0x0004)
    settings[offset++] = 0x00; settings[offset++] = 0x04;
    int window_size = 32768 + (fast_rand() % 32768); // 32KB-64KB
    settings[offset++] = (window_size >> 8) & 0xFF; settings[offset++] = window_size & 0xFF;
    
    // Setting 5: MAX_FRAME_SIZE (0x0005)
    settings[offset++] = 0x00; settings[offset++] = 0x05;
    int frame_size = 16384 + (fast_rand() % 16384); // 16KB-32KB
    settings[offset++] = (frame_size >> 8) & 0xFF; settings[offset++] = frame_size & 0xFF;
    
    // Setting 6: MAX_HEADER_LIST_SIZE (0x0006)
    settings[offset++] = 0x00; settings[offset++] = 0x06;
    int max_header_list = 8192 + (fast_rand() % 16384); // 8KB-24KB
    settings[offset++] = (max_header_list >> 8) & 0xFF; settings[offset++] = max_header_list & 0xFF;
...
void randomize_headers_v18tls(Connection *conn) {
    ...
    int lang_idx = fast_rand() % (sizeof(accept_language_pool) / sizeof(accept_language_pool[0]));
    const char *accept_lang = accept_language_pool[lang_idx];
    
    // Shuffle header order using random indices
    int header_order[] = {0, 1, 2, 3, 4, 5};
    for (int i = 0; i < 6; i++) {
        int j = fast_rand() % 6;
        int tmp = header_order[i];
        header_order[i] = header_order[j];
        header_order[j] = tmp;
    }
    
    for (int i = 0; i < 6; i++) {
        switch (header_order[i]) {
            case 0:
    ...
    // Add additional spoofing headers for geo-IP bypass
    if (fast_rand() % 2) {
        len += snprintf(buf + len, sizeof(buf) - len,
    ...
    strncpy(conn->randomized_headers, buf, sizeof(conn->randomized_headers) - 1);
    conn->randomized_headers[sizeof(conn->randomized_headers) - 1] = '\0';
}

int get_timing_jitter_ms() {
    // Browser-like timing: 100-500ms between requests
    return 100 + (fast_rand() % 400);
}
...
#pragma GCC diagnostic pop
    ...
    if (args.is_v18_tls || args.is_v5_rapid || args.is_v6_void || args.is_v8_phantom || args.is_v7_pipe || args.is_v12_eclipse || args.is_v14_phantom || args.is_v20_ws) {
        ...
        SSL_CTX_set1_sigalgs_list(ssl_ctx, "ecdsa_secp256r1_sha256:rsa_pss_rsae_sha256:rsa_pss_pss_sha256:rsa_pkcs1_sha256:ecdsa_secp384r1_sha384:rsa_pss_rsae_sha384:rsa_pss_pss_sha384:ecdsa_secp521r1_sha512");
        SSL_CTX_set1_groups_list(ssl_ctx, "X25519:P-256:P-384:P-521");
